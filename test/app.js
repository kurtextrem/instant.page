...
let PORT = Number.parseInt(process.argv[argvIndexOfFile + 1])
if (isNaN(PORT)) {
	PORT = 8000
}
...
function handleCookies(req) {
	...
	cookies.split('; ').map(cookie => {
		...
		const cookieValueSplit = value.split(',').map(param => Number.parseInt(param))
...
async function requestListener(req, res) {
	...
	const pathString = req.url.slice(1)
	let page = Number.parseInt(pathString)
	if (pathString == '') {
		page = 1
	}
